...
$(function () {
    ...
    */
    ...
    if (storeCode == 'regional') {
        $.getJSON(baseUrl+'dboard/getdataregional/'+actYear+'/'+actMonth+'/'+encodeURIComponent(regionalCode), function(json) {
            ...
            options.title.text = 'Expense Report by Regional (Yogya)';
            options.subtitle.text = 'Regional ' + json['group_name'];
            options.legend.title.text = json['data_label'];
            
            options.plotOptions.series.cursor = "pointer";
            options.plotOptions.series.point.events.click = function() {
                location.href = baseUrl + this.options.url;
            };
    ...
    else if (storeCode == 'cluster') {
        $.getJSON(baseUrl+'dboard/getdatacluster/'+actYear+'/'+actMonth+'/'+encodeURIComponent(clusterCode), function(json) {
            ...
            options.title.text = 'Expense Report by Cluster (Yomart)';
            options.subtitle.text = 'Cluster ' + json['group_name'];
            options.legend.title.text = json['data_label'];
        ...
        $.getJSON(baseUrl+'dboard/getdata/'+actYear+'/'+actMonth+'/'+storeCode, function(json) {
            options.xAxis.categories = json['expense_name'];
            options.series[0] = json['expense_pc'];
            
            options.title.text = 'Expense Report by Store';
            //options.subtitle.text = 'Source: ORAFIN';
