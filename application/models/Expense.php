...
class Expense extends CI_Model {
        ...
        public function loadClusterArray($pYear, $pMonth, $pCluster) {
                ...
                foreach($queryCluster->result() as $rowCluster) {
                        ...
                        $sql = "select load_expense_v2(?, ?, ?, 'v_expense'); fetch all in v_expense;";  
                        ...
                        foreach($query->result() as $row) {
                                ...
                                # use additional info
                                if ($row->is_dc) 
                                        $expenseNamePc[$row->xpense_name]["pc"] = round((100 * $row->amount / $row->total_expense), 2);
                                else
                                        $expenseNamePc[$row->xpense_name]["pc"] = round((100 * $row->amount / $row->total_netsales), 2);  
        ...
        public function loadYtdArrayV2($pStore, $pYear) {
                $currYear = $pYear;
                $checkYear = date('Y');
                if ($pYear == $checkYear) {
                        $currTime = strtotime('-2 months');
        ...
        public function loadArrayV2($pYear, $pMonth, $pStore) {
                $currTime = strtotime('-2 months');
                $currYear = date('Y', $currTime);
                $currMonth = date('n', $currTime);
                ...
                foreach($query->result() as $row) {
                        ...
                        # use additional info
                        if ($row->is_dc) 
                                $expensePc["data"][$j]["y"] = round((100 * $row->amount / $row->total_expense), 2);
                        else
                                $expensePc["data"][$j]["y"] = round((100 * $row->amount / $row->total_netsales), 2);
